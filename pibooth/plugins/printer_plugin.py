...
class PrinterPlugin(object):
    ...
    def print_previous_picture(self, cfg, app):
        LOGGER.info("Send previous picture to printer")
        app.printer.print_file(app.previous_picture_file,
                               cfg.getint('PRINTER', 'pictures_per_page'))
        app.count.printed += 1

    def print_previous_previous_picture(self, cfg, app):
        LOGGER.info("Send previous previous picture to printer")
        app.printer.print_file(app.previous_previous_picture_file,
                               cfg.getint('PRINTER', 'pictures_per_page'))
        app.count.printed += 1
    ...
    def state_wait_do(self, cfg, app, events):
        if app.printer.is_installed():
            if app.find_right_event(events) and app.previous_picture_file :
                ...
                self.print_previous_picture(cfg, app)

            elif app.find_left_event(events) and app.previous_previous_picture_file :
                if not app.printer.is_ready():
                    LOGGER.warning("Maximum number of printed pages reached (%s/%s max)", app.count.printed,
                                   cfg.getint('PRINTER', 'max_pages'))
                    return

                self.print_previous_previous_picture(cfg, app)
